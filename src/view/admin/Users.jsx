...
function Users() {
	...
	const getInfoById = function (id, it, table) {
		const filteredInfo = all[table].filter((info) => info.id === id);
		if (filteredInfo.length > 0) {
			return filteredInfo[0][it];
		} else {
						...
						<div className={showDetailRol[params.row.id] ? "fixed" : "hidden"}>
							<div className="bg-[#000] bg-opacity-50 rounded-md shadow-md p-2">
								<p className="text-[#fff]">
									{getInfoById(params.value, "rol", "roles")}
								</p>
								<p className="text-[#fff]">
									{getInfoById(params.value, "description", "roles")}
								</p>
							</div>
						</div>
					</div>
